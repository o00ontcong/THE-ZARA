    ...
    func geocodeAddress(address: String!, withCompletionHandler completionHandler:((status: String, success: Bool)->Void)) {
        if let lookupAddress = address {
            ...
            dispatch_async(dispatch_get_main_queue(), {
                ...
                do {
                    let myDictionary: Dictionary<NSObject, AnyObject> = try NSJSONSerialization.JSONObjectWithData(geocodingResultsData!, options: NSJSONReadingOptions.MutableContainers) as! Dictionary<NSObject, AnyObject>
                    
                    let status = myDictionary["status"] as! String
                    if status == "OK" {
                        let allResults = myDictionary["results"] as! Array<Dictionary<NSObject, AnyObject>>
